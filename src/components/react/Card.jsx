export function Card({project}){
  ...
  return (
    ...
    <div className="border-2 border-dashed h-100 bg-sky-900 max-w-sm rounded overflow-hidden shadow-lg flex flex-col card">
      ...
      <div className="px-6 pt-3 pb-2 tags">
        {tags.map((tag, index) => (        
          <span key={index} className="inline-block bg-gray-200 rounded-full px-3 py-1 text-sm font-semibold text-gray-700 mr-2 mb-2 tag">{tag}</span>
        ))}
     </div>
    </div>
    </>
